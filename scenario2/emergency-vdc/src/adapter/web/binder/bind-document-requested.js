...
const TimePeriod = require('../../../domain/model/time-period'),
    ...
    DocumentRequested = require('../../../domain/event/document-requested');

function bindDocumentListRequested(req) {
    let validated = R.lift(R.construct(DocumentRequested))(
        ...
        validatePosition(req.params.position)
    );
...
function validatePosition(position) {
    if (!position) return Validation.Failure(['position shuold be a positive integer']);
    if (isNaN(position)) return Validation.Failure(['position should be a positive integer']);
    let intPosition = parseInt(position);
    if (intPosition < 0) return Validation.Failure(['position should be a positive integer']);
    return Validation.Success(intPosition);
}
